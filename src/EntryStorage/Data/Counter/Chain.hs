module EntryStorage.Data.Counter.Chain
( Type ) where

import qualified EntryStorage.Data.Counter.Link.Backward as BackwardLink
...
instance Serializable.Interface Type where
    ...
    deserialize words
        ...
        | otherwise 
        = Left $ "Wrong number of words when deserializing counter chain - " ++ (show . length) words ++ " Starting with: " ++ (show . take 2) words
